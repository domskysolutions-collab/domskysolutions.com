...
const env={KIT_API_KEY:'unit-test-only',KIT_STACK_FORM_ID:'123',KIT_STACK_SEQUENCE_ID:'456',KIT_STACK_TAG_IDS:JSON.stringify(Object.fromEntries(segments.map((s,i)=>[s,i+1])))};
...
const success=(async (url,options)=>{
  const request={url:String(url),body:JSON.parse(String(options?.body)),headers:options?.headers};requests.push(request);
  const isCreate=request.url.endsWith('/v4/subscribers');
  return new Response(JSON.stringify({subscriber:{id:1,state:'inactive'}}),{status:isCreate?201:200});
}) as typeof fetch;
...
assert.equal(requests[0].body.fields.stack_result,'Lean Content Builder');
assert.equal(requests[0].headers['X-Kit-Api-Key'],'unit-test-only');
assert(!('businessOther' in requests[0].body.fields));
assert(requests.some(r=>r.url.endsWith('/forms/123/subscribers/1')));
assert(requests.some(r=>r.url.endsWith('/sequences/456/subscribers/1')));
assert.equal(requests.filter(r=>r.url.includes('/tags/')).length,4);
