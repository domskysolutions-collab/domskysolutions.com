...
import { calculateSaasAudit, currencies, describeMonthlyResult, formatCurrency, parseSaasAuditValues, type Currency, type SaasAuditField } from '../../data/saasAudit';

const cashFields: Array<{ key: SaasAuditField; label: string; help: string }> = [
  ...
  { key: 'externalServiceCash', label: 'External service cash', help: 'Consultants, accountants or other outside services. Reported separately; not counted as recurring software reduction.' },
];

export const SaasCalculatorPage = () => {
  const [currency, setCurrency] = useState<Currency>('EUR');
  const [values, setValues] = useState<Record<SaasAuditField, string>>({ oldRecurringCash: '104', retainedRecurringCash: '74', replacementRecurringCash: '0', oneTimeMigrationCash: '30', externalServiceCash:'0' });
  const [time, setTime] = useState({ before: '', after: '', migration: '' });
  const parsed = useMemo(() => parseSaasAuditValues(values), [values]);
  const result = parsed.input ? calculateSaasAudit(parsed.input) : null;
  const money = (value: number) => formatCurrency(value, currency);
  const signedMoney = (value: number) => `${value > 0 ? '+' : value < 0 ? '−' : ''}${money(value)}`;
  const resultTone = !result ? 'border-red-500/40 bg-red-500/10 text-red-300' : result.monthlyNetReduction > 0 ? 'border-brand-cyan/40 bg-brand-cyan/10 text-brand-cyan' : result.monthlyNetReduction < 0 ? 'border-brand-amber/40 bg-brand-amber/10 text-brand-amber' : 'border-gray-700 bg-white/[0.03] text-white';
        ...
        <div className="mt-12 grid gap-8 lg:grid-cols-[1.05fr_.95fr]">
          ...
          <aside aria-live="polite" className="space-y-6">
            <section className={`rounded-2xl border p-6 md:p-8 ${resultTone}`}>
              ...
              {result ? <><p className="text-4xl font-bold md:text-5xl">{signedMoney(result.monthlyNetReduction)}<span className="ml-2 text-base font-normal opacity-75">/ month</span></p>
              ...
              <dl className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
                ...
                <div className="rounded-xl bg-black/20 p-4 sm:col-span-2 lg:col-span-1 xl:col-span-2"><dt className="text-sm opacity-70">External services, reported separately</dt><dd className="mt-1 text-2xl font-bold">{money(result.externalServiceCash)}</dd></div>
              </dl></> : <p role="alert" className="leading-7">Correct the highlighted values to calculate this scenario.</p>}
            </section>
