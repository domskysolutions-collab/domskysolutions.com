...
function getReport(answers: Answers) {
  ...
  return { starter, workflow, avoid: budget === 'lean' ? 'Avoid stacking multiple premium assistants before one becomes part of your daily workflow.' : 'Avoid adding another general-purpose assistant until you have measured the overlap in your current stack.' };
}
...
export function StackScorecardPage() {
  ...
  return <main className="min-h-screen bg-grid-pattern pt-28 pb-20 px-4 sm:px-6">
    <div className="max-w-4xl mx-auto">
      ...
      {step === questions.length && status !== 'success' && <section className="max-w-2xl mx-auto bg-brand-surface border border-brand-border rounded-2xl p-6 sm:p-10"><div className="flex items-center gap-2 text-brand-cyan font-mono text-sm mb-4"><Check size={18}/> Your scorecard is ready</div><h2 className="text-2xl text-white font-bold mb-3">Where should I send your personalized report?</h2><p className="text-gray-400 mb-7">See your recommended starter stack, subscription-overlap check, next workflow, and tools to avoid.</p><form onSubmit={submit} className="space-y-4"><label className="block text-sm text-gray-300" htmlFor="scorecard-email">Email address</label><div className="flex flex-col sm:flex-row gap-3"><input id="scorecard-email" type="email" required value={email} onChange={e=>setEmail(e.target.value)} placeholder="you@example.com" className="flex-1 rounded-lg bg-black/30 border border-white/15 px-4 py-3 text-white"/><button disabled={status==='loading'} className="rounded-lg bg-brand-cyan text-brand-bg px-5 py-3 font-bold hover:bg-brand-amber transition-colors">{status==='loading' ? 'Sending…' : 'Email my report'}</button></div><label className="flex gap-3 items-start text-sm text-gray-400"><input type="checkbox" checked={newsletter} onChange={e=>setNewsletter(e.target.checked)} className="mt-1 accent-brand-cyan"/>Yes, send me The Weekly Edge newsletter too. Unsubscribe anytime.</label>{status==='error' && <p role="alert" className="text-red-400 text-sm">I could not send that. Please check the address and try again.</p>}</form></section>}
      {status === 'success' && <section className="max-w-3xl mx-auto"><div className="bg-brand-surface border border-brand-cyan/40 rounded-2xl p-7 mb-6"><div className="flex items-center gap-2 text-brand-cyan font-mono text-sm mb-3"><Mail size={17}/> Report sent</div><h2 className="text-2xl text-white font-bold">Your recommended starter stack</h2><div className="grid sm:grid-cols-3 gap-3 mt-5">{report.starter.map(tool=><div key={tool} className="rounded-lg bg-black/20 border border-white/10 p-4 text-gray-200">{tool}</div>)}</div><p className="text-gray-300 mt-6">No savings estimate is calculated without your actual current, retained, replacement and migration costs.</p></div><div className="grid sm:grid-cols-2 gap-4"><div className="bg-brand-surface border border-white/10 rounded-xl p-5"><h3 className="text-white font-bold mb-2">Automate next</h3><p className="text-gray-400 text-sm">{report.workflow}</p></div><div className="bg-brand-surface border border-white/10 rounded-xl p-5"><h3 className="text-white font-bold mb-2">Be careful with</h3><p className="text-gray-400 text-sm">{report.avoid}</p></div></div><p className="text-center text-gray-500 text-sm mt-8">Want to explore specific tools? <Link to="/tools" className="text-brand-cyan underline">Browse the free tools</Link>.</p></section>}
    </div>
  </main>;
}
