    ...
    <div className="lf-shell">
      ...
      {result && <div className="lf-results" data-full={stage === 'full' ? 'true' : 'false'}>
        ...
        {stage === 'partial' ? <>
          ...
          <h3 className="lf-section-title">Your keep, trial, add or skip plan</h3>
          <div className="lf-products">{result.essentials.map((item,index) => <article className="lf-product" key={item.category}><div className="lf-product-top"><span>0{index+1} / {item.categoryName}</span><span>{item.decision === 'keep' ? 'Keep' : item.decision === 'trial' ? 'Trial' : item.decision === 'add' ? 'Add if trial passes' : 'Skip purchase'}</span></div><h4>{item.name}</h4><p className="lf-price">{item.price}</p><p>{item.reason}</p><dl><dt>Current gap</dt><dd>{item.gap}</dd><dt>Trial rule</dt><dd>{item.trial}</dd><dt>Incremental cost</dt><dd>{item.incrementalCost}</dd></dl>
